...
public class RoundService {
    ...
    public void putTeamsInFirstRound(Round firstRound, Map<Seed, Team> tournamentSeeding) {
        this.doubleElimination.getSeedToMatchRelations()
                .forEach(rel -> allocateTeamByRelation(firstRound, tournamentSeeding, rel));
    }

    private void allocateTeamByRelation(Round firstRound,
                                        Map<Seed, Team> tournamentSeeding,
                                        com.volleyservice.SystemDataProvider.SeedToMatchRelation rel) {
        findMatchByNumber(firstRound, rel.getMatchNumber())
                .setTeams(getTeamsByRelations(tournamentSeeding, rel));
    }

    private List<Team> getTeamsByRelations(Map<Seed, Team> tournamentSeeding,
                                           com.volleyservice.SystemDataProvider.SeedToMatchRelation rel) {
        return rel.getSeeds().stream()
